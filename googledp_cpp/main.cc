...
std::vector<double> load_column_from_csv(const std::string& file_path, const std::string& col_name) {
...
struct CsvWriter {
    ...
    explicit CsvWriter(const std::string& p)
        : path(p),
          header_written(std::filesystem::exists(p) && std::filesystem::file_size(p) > 0) {}

    void ensure_header() {
        if (header_written) return;
        std::ofstream out(path, std::ios::app);
        out << "library,language,dataset_name,dataset_size,query,mechanism,epsilon,mae,avg_time_ms,runs,lower,upper\n";
        header_written = true;
    }
...
int main() {
    ...
    const std::vector<double> EPSILON_VALUES = {0.1, 0.5, 1.0, 1.5, 2.0, 2.5, 3.0};
    ...
    for (const auto& config : datasets_to_test) {
        ...
        std::vector<double> numbers = load_column_from_csv(config.file, SUM_COLUMN);
        ...
        for (double epsilon : EPSILON_VALUES) {
            ...
            for (int i = 0; i < NUM_RUNS; ++i) {
                auto count_statusor = differential_privacy::Count<int64_t>::Builder()
                                          .SetEpsilon(epsilon)
                                          .Build();

                if (!count_statusor.ok()) {
                    std::cerr << "Error building Count: " << count_statusor.status() << "\n";
                    return 1;
                }
                auto count_dp = std::move(count_statusor).value();

                auto result_statusor = count_dp->Result(numbers.begin(), numbers.end());
                if (!result_statusor.ok()) {
                    std::cerr << "Error computing Count result: " << result_statusor.status() << "\n";
                    return 1;
                }
                auto result = std::move(result_statusor).value();

                noisy_counts.push_back(differential_privacy::GetValue<int64_t>(result));
            }

            auto end_time = std::chrono::high_resolution_clock::now();
            double avg_time = std::chrono::duration<double>(end_time - start_time).count() / NUM_RUNS;
        ...
        for (double epsilon : EPSILON_VALUES) {
            ...
            for (int i = 0; i < NUM_RUNS; ++i) {
                auto sum_statusor = differential_privacy::BoundedSum<double>::Builder()
                                        .SetEpsilon(epsilon)
                ...
                if (!sum_statusor.ok()) {
                    std::cerr << "Error building BoundedSum: " << sum_statusor.status() << "\n";
                    return 1;
                }
                auto sum_dp = std::move(sum_statusor).value();

                auto result_statusor = sum_dp->Result(numbers.begin(), numbers.end());
                if (!result_statusor.ok()) {
                    std::cerr << "Error computing Sum result: " << result_statusor.status() << "\n";
                    return 1;
                }
                auto result = std::move(result_statusor).value();

                noisy_sums.push_back(differential_privacy::GetValue<double>(result));
            }
